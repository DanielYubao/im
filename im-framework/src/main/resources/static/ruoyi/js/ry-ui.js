    ...
    $.extend({
    	...
    	table: {
            ...
            init: function(options) {
                ...
                _pageList=$.common.isEmpty(options.pageList)?[10, 25, 50]:options.pageList;
                $('#bootstrap-table').bootstrapTable({
                    ...
                    pageSize: $.common.isEmpty(options.pageSize) ? 10 : options.pageSize,                                       // 每页的记录行数（*）
                    pageList: _pageList,                             // 可供选择的每页的行数（*）
                    ...
                    responseHandler: $.common.isEmpty(options.callback)? $.table.responseHandler:options.callback        // 回调函数
                });
            },
            // 查询条件
    		...
    		exportExcel: function(formId) {
    			...
    			if(rows.length!=0){
    				var data = { "name":'params[ids]',"value": rows.join()};
    				array.push(data);
    			}
    			$.post($.table._option.exportUrl,array, function(result) {
    	...
    	modal: {
			...
			layerOption : {
                ...
                yes: function(index, layero) {
                    ...
                    if(typeof(iframeWin.contentWindow.getSubmitFlag) == "function"){
                        submitFlag = iframeWin.contentWindow.getSubmitFlag();
                    }
                    if (submitFlag == "noSubmit"){
                        iframeWin.contentWindow.submitHandler();
                    }
                },
            ...
            open: function (title, url, width, height) {
                ...
                $.modal.layerOption.content = url;
                $.modal.layerOption.title= title;
                $.modal.layerOption.area = [width + 'px', height + 'px']

            	layer.open($.modal.layerOption);
            },
            // 弹出层指定参数选项
            openOptions: function (options) {
                ...
                layer.open({
                    ...
                    title: _title,
					fix: false,
					btnAlign: options.btnAlign || 'r',
					area: [_width + 'px', _height + 'px'],
                    content: _url,
                    shadeClose: true,
					btn: (options.btn == undefined || options.btn == null)? ['<i class="fa fa-check"></i> 确定', '<i class="fa fa-close"></i> 取消'] : options.btn,
					success: function(layero, index){
						layer.iframeAuto(index);
					},
                    yes: (options.yes == undefined || options.yes == null)? function (index, layero) {
                        options.callBack(index, layero);
                    }:options.yes,
					cancel:(options.cancel == undefined || options.cancel == null)? function () {
                        return true;
                    }:options.cancel
                });
            },
        ...
        operate: {
        	// 提交数据
        	submit: function(url, type, dataType, data, errcallback) {
            	...
            	var config = {
        	        ...
        	        success: function(result) {
                        $.operate.ajaxSuccess(result, errcallback);
        	        }
        	    };
        	    $.ajax(config)
            },
			...
			redirect:function(id){
        		var url=$.table._option.redirecturl.replace("{id}", id);
                $.modal.open("专家信息" , url);
            },
            // 修改信息
            edit: function(id) {
            ...
            // 保存结果弹出msg刷新table表格
            ajaxSuccess: function (result, errcallback) {
        ...
        tree: {
        	...
        	init: function(options) {
        		...
        		var _expandAllNodes = $.common.isEmpty(options.expandAllNodes) ? true : options.expandAllNodes;
                ...
                var view = {
                    selectedMulti: false, nameIsHTML: true
				}
                var finalCallback = $.extend({},callback,options.callback)
				var finalView = $.extend({},view,options.view)
        	    var setting = {
        	    	check: options.check,
        	        view: finalView,
				...
				$.get(options.url, function(data) {
        			...
        			// 扩展是否展开所有树节点
                    tree.expandAll(_expandAllNodes);
        		}, null, null, "正在加载，请稍后...");
        	    return treeObj;
        	},
        	// 根据Id和Name选中指定节点
