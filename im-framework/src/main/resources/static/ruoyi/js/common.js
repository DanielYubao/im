...
function closeMenuItem(dataUrl) {
	...
	if( dataUrl == undefined || $.trim(dataUrl).length == 0) return false;
	var topWindow = $(window.parent.document);
	$(".menuTab",topWindow).each(function(index){
		if (dataUrl.indexOf($(this).data('id')) != -1){
		// if (dataUrl.endsWith($(this).data('id'))) {
			$(".menuTab",topWindow)[index-1].click();
			$(this).remove(); // 移除选项卡
			$(".RuoYi_iframe [data-id='"+ $(this).data('id') +"']",topWindow).remove(); // 移除类容
		}
	});
}

function refressMenuItem(dataUrl) {
	var flag = false;
	if( dataUrl == undefined || $.trim(dataUrl).length == 0) return false;
	var topWindow = $(window.parent.document);
	$(".menuTab",topWindow).each(function(index){
		if (dataUrl.indexOf($(this).data('id')) != -1){
	    	var target = $('.RuoYi_iframe[data-id="' + $(this).data('id') + '"]',topWindow);
...
$.ajaxSetup({
    complete: function(XMLHttpRequest, textStatus) {
		if ((XMLHttpRequest.responseJSON && XMLHttpRequest.responseJSON.code == 403) || textStatus == "parsererror") {
...
function selectDictLabelText(datas, value) {
    var actions = [];
