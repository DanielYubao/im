...
$(document).ready(function(){
	...
	jQuery.validator.addMethod("issame",function(){
		...
		//部门名称不能超过20个字
	jQuery.validator.addMethod("checkDptName",function(){
		var dptName =$("#deptName").val();
		return dptName.length <= 20
		},'"部门名称"限制20个字符以内');
		
		//登录名称需要6-18个字符内
	jQuery.validator.addMethod("checkLoginName",function(){
		var loginName =$("#loginName").val();
		return (loginName.length <= 18 && loginName.length >= 6)
		},'需要6-18个字符内');
	...
	$("#basicInfoForm").validate({
		...
		messages:{
			...
			dptName:{
				required:"请输入部门名称",
				dptName:'"部门名称"限制20个字符以内'
			},
			loginName:{
				required:"请输入部门名称",
				loginName:'登录名称需要6-18个字符内'
			}
		},
